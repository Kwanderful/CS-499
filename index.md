...
Throughout my time at SNHU, I gained valuable skills that will help me become a better developer. I started out self-taught and learned how to create my own applications but was missing important steps like security and just general good programming practices. At SNHU, I learned about the software developer lifecycle and how one might perform a cycle of coding and testing throughout the development process. Consistent testing ensures that if there are bugs in the code, they are unlikely to be saved until the end of development when the amount of code and files to sift through becomes too overwhelming. Debugging and security testing as you go means that you can more easily identify issues and resolve them. Waiting too long might make it difficult to find where the problem is, especially if they don’t generate warnings like logic errors might. 

Since I want to be a full-stack developer, I greatly enjoyed the full-stack class I took. I already had experience using React and MongoDB, but this class taught me about Angular which is another popular front-end framework. This class gave me the confidence for one of my enhancements: turning a couple of simple files into a full-stack application. I wanted to make sure I really understood full-stack fundamentals, and while I learned some of them myself, SNHU taught me more. Separating client and server makes sure that sensitive information stays safe from those who view the website. Again, this is part of the security mindset that I developed over the years in this program. 

In the software development lifecycle course, I learned about how one might collaborate with other developers on a team. Having someone check your code over can be helpful as sometimes a new set of eyes is needed to identify issues and errors. It’s also important to code in such a way that someone can easily look at your code and understand its goal and purpose. Because of this, I try to write cleaner code with variable names that make sense and a style that stays consistent throughout. If code is repeated often throughout the program, it could be beneficial to turn it into a reusable function instead, saving space and cutting down on the amount of bloat. 

In the reverse engineering course, I learned about turning binary files into assembly and then turning that assembly back into C++. I never realized how much information you could get out of disassembling source code and realized the security issues around including sensitive information in plain text. Having environment files or a secrets manager that hold variables containing sensitive information is a way to secure that information. These files are normally not added to repositories and version control, and this is true for my repositories as well. 

In this portfolio and my enhancements, I tried to demonstrate the skills I learned in this program. The most important skill, in my opinion, is security. I focused on improving the security in my artifacts by removing sensitive information and adding authentication through the use of a database. I separated server and client information to further keep information safe, such as data retrieved directly from the database. I also focused on performance by choosing algorithms that made sense and were efficient for the scope of the project. In a situation where it’s expected that a lot of data might be added, algorithms like selection sort become worse. I wrote my code with the assumption that others might one day look at it and tried to write comments only when needed to not bloat the files too much. I find that when a piece of code is written well, it might not even need a comment to describe what it’s doing.  
...
#### 1. Employ strategies for building collaborative environments that enable diverse audiences to support organizational decision making in the field of computer science
I turned an idea into a website that allows users to interact with it fully. The functionality desired in the ContactService class was integrated into a web application that lets users manage their contacts. The user interacts with the client-side interface I created, and those interfaces interact with the server which interacts with the database. Separating client and server keeps data secure and gives a user-friendly appearance to the database operations.
