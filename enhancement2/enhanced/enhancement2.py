...
def SortByClientID(client):
    return client["clientID"]

def SortByFirstName(client):
    return client["firstName"]

def SortByLastName(client):
    return client["lastName"]

def DisplayInfo():
    client_array = helper_functions.GetClientArray(client_collection)
    choice = 0
    while choice != "1" and choice != "2" and choice != "3":
        # Sort according to user preference
        choice = input("Sort by [1] Client ID [2] First Name or [3] Last Name?\n")
        if choice == "1":
            client_array.sort(key=SortByClientID)
        elif choice == "2":
            client_array.sort(key=SortByFirstName)
        elif choice == "3":
            client_array.sort(key=SortByLastName)
        else:
            print("Please enter either 1 or 2.\n")

    print("ID  Client's Name    Service Selected (1 = Brokerage, 2 = Retirement)")
